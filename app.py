...
st.caption("ระบุรายชื่อหุ้นหรือ ETF เพื่อดึงราคารอบปัจจุบัน (รองรับความละเอียดสูงสุด 9 ตำแหน่ง ทั้งจำนวนหุ้นและต้นทุนเฉลี่ย)")

# รายการสินทรัพย์เริ่มต้น (Default Assets) - โครงสร้างไวยากรณ์สมบูรณ์แบบ
default_assets = [
...
for i, (def_sym, def_qty, def_cost) in enumerate(default_assets):
    ...
    with cols[1]:
        qty = cols[1].number_input(f"Qty {i+1}", value=float(def_qty), min_value=0.0, step=0.000000001, format="%.9f", key=f"qty_{i}")
    with cols[2]:
        cost = cols[2].number_input(f"Cost {i+1}", value=float(def_cost), min_value=0.0, step=0.000000001, format="%.9f", key=f"cost_{i}")
    with cols[3]:
        if sym:
...
if input_assets:
    with st.spinner("🏰 กำลังคำนวณมูลค่าป้อมปราการความละเอียดสูง..."):
        for item in input_assets:
            ...
            portfolio_details.append({
                ...
                "จำนวนหุ้น": f"{qty_float:.9f}",
                "ต้นทุนเฉลี่ย": f"{cost_float:.9f}",
