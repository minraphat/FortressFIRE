...
# Custom Minimalist Theme using CSS
st.markdown("""
    <style>
...
def get_usd_thb_rate():
    try:
        ...
        # ใช้ประวัติราคาวันล่าสุดแทนเพื่อความชัวร์ ป้องกันตัวแปร fast_info เอ๋อ
...
input_assets = []
default_assets = [("VOO", 15, 420.0), ("SCHD", 40, 75.0), ("PTT.BK", 1500, 34.0), ("", 0, 0.0), ("", 0, 0.0)]
...
# --- 5. CALCULATION LOGIC WITH DATA CLEANING ---
portfolio_details = []
total_stock_value_thb = 0
total_cost_thb = 0

if input_assets:
    with st.spinner("🏰 กำลังตรวจสอบและคำนวณมูลค่าป้อมปราการ..."):
        for item in input_assets:
            ...
            try:
                # แก้บั๊ก TypeError: ดึงราคาวันล่าสุดเพื่อความชัวร์และแปลงเป็น float
                ...
                st.error(f"⚠️ ดึงราคา {item['symbol']} สดๆ ไม่สำเร็จ ระบบจะใช้ราคาทุนแทนชั่วคราว")

            # แปลงมูลค่าให้เป็น float มั่นใจว่าคูณกันไม่พังแน่นอน
